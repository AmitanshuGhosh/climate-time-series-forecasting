...
|---|---|---|---|
| 0 | Data acquisition + sanity checks | Verified real data before any analysis | ✅ done |
| 1 | Exploratory plots + basic statistics | See the data before modelling it | ✅ done (`experiments/explore.py`) |
| 2 | Trend analysis: Theil-Sen slope + Mann-Kendall test on temperature (and on CO2) | Quantify warming with a significance test, the standard climatology method | ✅ done (`experiments/trend.py`) |
| 3 | Breakpoint analysis: warming rate before/after ~1970 | Is warming accelerating? | ✅ done (`experiments/breakpoint.py`) |
| 4 | Seasonal decomposition of CO2 (STL) | Separate trend, seasonal cycle, residual; measure the cycle's growing amplitude | ✅ done (`experiments/stl_co2.py`) |
| 5 | Periodicity: Lomb-Scargle/FFT on sunspots | Find the ~11-year solar cycle in the data (the same period-finding skill used for exoplanet transits) | ✅ done (`experiments/periodicity.py`) |
| 6 | Attribution: partial correlations of temperature vs CO2 and vs sunspot cycle | Do the data separate the two candidate drivers? | ✅ done (`experiments/attribution.py`) |
| 7 | Anomaly events: El Nino years, volcanic years (Pinatubo dip) | Real-world events should show up as outliers | ✅ done (`experiments/events.py`) |
| 8 | Forecasting: CO2 and temperature (ARIMA vs XGBoost on lags vs LSTM) | Forecast the near future; compare methods honestly | ✅ done (`experiments/forecast.py`) |
| 9 | Report (HTML, research-paper style, embedded figures) | Present the findings | ✅ done (`experiments/report.py` -> `results/report.html`) |
